...
// --- Config ---
const SECRET = "IXL_CHEAT_SECRET_2024"; // keep this constant
const ADMIN_USER = "admin";
const ADMIN_PASS = "password123"; // change to your password
...
// --- Generate key with prefix "Tempest-" ---
app.post('/api/generate-key', auth, (req, res) => {
    ...
    const id = crypto.randomBytes(4).toString('hex'); // 8-char short id
...
// --- Validate key (accepts with or without prefix) ---
app.get('/api/validate-key', (req, res) => {
    let { key } = req.query;
    if (!key) return res.status(400).json({ error: 'Missing key' });

    // Strip prefix if present
    if (key.startsWith('Tempest-')) {
        key = key.slice('Tempest-'.length);
    }
